...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="""Generate C code for Butterworth IIR digital filters.""",
                                     ...
                                     epilog=usage_guide)
    parser.add_argument('--type',  default='bandpass',  type=str,
                        choices = ['lowpass', 'highpass', 'bandpass', 'bandstop'], 
                        help = 'Filter type: lowpass, highpass, bandpass, bandstop (default lowpass).')
    
    parser.add_argument('--rate',  default=800,  type=float, help = 'Sampling frequency in Hz (default 10).')
    parser.add_argument('--order', default=4,   type=int,   help = 'Filter order (default 4).')
    parser.add_argument('--freq',  default=202.5, type=float, help = 'Critical frequency (default 1.0 Hz).')
    parser.add_argument('--width', default=355.0, type=float, help = 'Bandwidth (for bandpass or bandstop) (default 1.0 Hz).')
